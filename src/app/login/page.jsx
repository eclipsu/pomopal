...
export default function Login() {
  ...
  const forceClaim = searchParams.get("setUsername") === "1";
  ...
  useEffect(() => {
    ...
    if (user && (!user.username || forceClaim)) {
      const suggestion = (user.name || "")
        ...
        .replace(/[^a-z]/g, "")
        .slice(0, 32);
      setUsernameDraft(user.username || suggestion);
      setNeedsUsername(true);
    }
  }, [user, userLoading, forceClaim]);
  ...
  const finishLogin = (nextUser) => {
    if (!nextUser?.username) {
      const suggestion = (nextUser?.name || "")
        .trim()
        .split(/\s+/)[0]
        .toLowerCase()
        .replace(/[^a-z]/g, "")
        .slice(0, 32);
      setUsernameDraft(suggestion);
      setNeedsUsername(true);
      return;
    }
  ...
  const saveUsername = async (e) => {
    e.preventDefault();
    const username = usernameDraft.toLowerCase().replace(/[^a-z]/g, "").slice(0, 32);
    if (!/^[a-z]{3,32}$/.test(username)) {
      setUsernameError("Use your first name in lowercase letters (3–32).");
      return;
    }
    setIsLoading(true);
    setUsernameError("");
    try {
      await axiosClient.patch("/user/username", { username });
      await refetch?.();
      router.push(returnTo);
    } catch (err) {
          ...
          {needsUsername ? (
            <>
              ...
              <p className="text-sm text-gray-400 mb-6">
                Use your first name in lowercase letters. Profile: /{usernameDraft || "…"}
              </p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setUsernameDraft(
                          e.target.value.toLowerCase().replace(/[^a-z]/g, "").slice(0, 32),
                        )
                      }
